...
def searh_fastq_files(target):
	...
	'''
	return:
		a list a absolute path of fastq files
	...
	if not len(sub_dirs) == 0:
		...
		# search sub dirs done. returns upper dir
...
def makeSampleList(dirname, namelist):
	...
	if len(fqfiles):
		...
		for sm in fqfiles:
			...
			samples.append(sample) # samples = ['R21022539-snOGseq8-Oggi6_combined_R1']
			suffixs.add(sf)

		if not len(suffixs) == 1:
			info('Fastq files are not consistent in suffixs. Please keep them in the same.', 'error')
			sys.exit()
...
def main(args=None):
	...
	if args.picard == None:
		...
		if checkUnixTools('picard'):
			genomelist['picard'] = os.popen('which picard').read().strip()
		else:
			info('No picard in your environment path. Please provide the full path to picard.jar', 'error')
			sys.exit()
	...
	if 'fq' not in cwd_files and 'fastq' not in cwd_files:
		info('Current work dirctory do not contains ./fastq or ./fq dir. Please check the path.', 'error')
		sys.exit()

	try:
		assert cwd_files.count('fq') + cwd_files.count('fastq') == 1
	except AssertionError:
		info('Seemingly you have two directories for fastq files, please put all them in one folder', 'error')
		sys.exit()
	...
	'bai="mapped/{sample}/{sample}_sorted_dedup.bai"',
	'output',
	'bed="beds/{sample}/{sample}.bed"',
	'fbed="beds/{{sample}}/{{sample}}.{}.bed"'.format(''.join(map(str, selectbases))),
	'threads',
	'1',
	'shell',
	'''"dmtools convertoBed -b {input.bam} -g {GENOME_FA} -o {output} --select_base {SELECTBASES} -q {MAPQ} --plot -t {threads}"'''
	]
	...
	'output',
	'"mapped/{{sample}}/{{sample}}.{}.bam"'.format(''.join(map(str, selectbases))),
	'shell',
	...
	'output',
	'"bigwigs/{{sample}}.rpkm.{}.bw"'.format(''.join(map(str, selectbases))),
	'threads',
	'5',
	'shell',

...
class SnakeBlock:
	...
	def constructVar(self, varlist):
		var = ""
		fasta_line = 'GENOME_FA = "{}"'.format(varlist.get('fasta'))
		tbit_line = 'GENOME_2BIT = "{}"'.format(varlist.get('tbit'))
		index_line = 'ALIGNER_INDEX = "{}"'.format(varlist.get('index'))
		picard_line = 'picard = "{}"'.format(varlist.get('picard'))
		mapq_line = 'MAPQ = {}'.format(varlist.get('mapq'))
		sbases_line = 'SELECTBASES = "{}"'.format(varlist.get('select_base'))


		var = '\n'.join(map(str, [fasta_line, tbit_line, index_line, picard_line, mapq_line, sbases_line])) + '\n'
